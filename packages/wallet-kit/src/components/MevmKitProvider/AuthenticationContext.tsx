...
interface MevmKitAuthenticationProviderProps<Message>
  extends AuthenticationConfig<Message> {
  enabled?: boolean;
  children: ReactNode;
}

export function MevmKitAuthenticationProvider<Message = unknown>({
  adapter,
  children,
  enabled = true,
  status
}: MevmKitAuthenticationProviderProps<Message>) {
