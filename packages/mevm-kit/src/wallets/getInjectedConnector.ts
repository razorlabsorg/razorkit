...
/*
 * Returns an injected provider that favors the namespace match, then flag match but falls back to window.ethereum
 */
...
function createInjectedConnector(provider?: any): CreateConnector {
  return (walletDetails: WalletDetailsParams) => {
    ...
    const injectedConfig = provider
      ? {
          target: () => ({
            id: walletDetails.mkDetails.id,
            name: walletDetails.mkDetails.name,
            provider,
          }),
        }
      : {};
