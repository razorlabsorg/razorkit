...
// If you want to declare it in the same file, do it at the top of the file.
declare global {
  namespace JSX {
    interface IntrinsicElements {
      gradient: ReactThreeFiber.Object3DNode<Gradient, typeof Gradient>;
    }
  }
}
...
function lerpRGB(color1: RGBColor, color2: RGBColor, t: number): RGBColor {
...
function circle(et: number, radius: number, acceleration: number): Vector2 {
...
class Gradient extends THREE.ShaderMaterial {
  constructor() {
    super({
      ...
      uniforms: {
        u_c1: { value: undefined },
        u_c2: { value: undefined },
        u_c3: { value: undefined },
        u_randomisePosition: { value: undefined },
        u_time: { value: 0 },
      },
    });
  }
  ...
  get u_randomisePosition(): Vector2 {
    return this.uniforms.u_randomisePosition.value;
  }
  set u_randomisePosition(v: Vector2) {
...
function Mesh(props: MeshProps) {
  ...
  useFrame(({ clock }) => {
    if (ref.current) {
      const material = ref.current.material as Gradient;
      const et = clock.getElapsedTime() + 40;
      const pos = circle(et, 2, 2);

      // Animate the gradient position
      material.u_time = pos[0];

      // Animate the mesh shape
      material.u_randomisePosition = [pos[0], pos[1]];

      // Animate colors
      material.u_c1 = lerpRGB(
        material.u_c1,
        props.u_c1,
        props.transitionSpeed,
      );
      material.u_c2 = lerpRGB(
        material.u_c2,
        props.u_c2,
        props.transitionSpeed,
      );
      material.u_c3 = lerpRGB(
        material.u_c3,
        props.u_c3,
        props.transitionSpeed,
      );
...
export function MeshGradient({ backgroundColor, u_c1, u_c2, u_c3 }: MeshGradientProps) {
