...
export type EthAuthenticationStatus =
  | 'loading'
  | 'unauthenticated'
  | 'authenticated';

export interface EthAuthenticationAdapter<Message> {
...
export interface EthAuthenticationConfig<Message> {
  adapter: EthAuthenticationAdapter<Message>;
  status: EthAuthenticationStatus;
}
...
const EthAuthenticationContext = createContext<EthAuthenticationConfig<any> | null>(
  null,
);

interface RazorKitEthAuthenticationProviderProps<Message>
  extends EthAuthenticationConfig<Message> {
  enabled?: boolean;
  children: ReactNode;
}

export function RazorKitEthAuthenticationProvider<Message = unknown>({
  adapter,
  children,
  enabled = true,
  status,
}: RazorKitEthAuthenticationProviderProps<Message>) {
  ...
  return (
    <EthAuthenticationContext.Provider
      value={useMemo(
...
export function useEthAuthenticationAdapter() {
  const { adapter } = useContext(EthAuthenticationContext) ?? {};
...
export function useEthAuthenticationStatus() {
  const contextValue = useContext(EthAuthenticationContext);
