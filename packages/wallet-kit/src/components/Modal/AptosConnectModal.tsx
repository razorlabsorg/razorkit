...
const Header = () => {
  return (
    <div className={'wkit-dialog__header'}>
      ...
      <Dialog.Close style={{ position: 'absolute', right: '16px', top: '16px' }} className={'wkit-dialog__close'}>
        <SvgClose />
      </Dialog.Close>
    </div>
  );
...
const Footer = () => {
  return (
    <div className={'wkit-new-to-movement'}>
      ...
      <a className={'wkit-new-to-movement__link'} href="https://docs.movementlabs.xyz" target="_blank">
        Learn More Here
      </a>
    </div>
  );
...
const WalletItem = (props: WalletItemProps) => {
  ...
  return (
    ...
    >
      <Icon icon={icon} className={'wkit-select-item__icon'} elClassName={'wkit-select-item__icon'} />
    ...
    <div className={'wkit-select__container'}>
      ...
      {isNonEmptyArray(props.wallets) &&
        props.wallets.map((wallet) => {
          return <WalletItem key={wallet.name} wallet={wallet} onSelect={props.onSelect} />;
        })}
    </div>
  );
};
...
const InstallGuide = (props: InstallGuideProps) => {
  ...
  return (
    <section>
      <div className={'wkit-dialog__header'}>
        <Dialog.Title className={'wkit-dialog__title'} style={{ margin: '-8px 12px -6px -8px' }}>
          <span className="wkit-dialog__close" onClick={props.onNavBack}>
            <SvgArrowLeft />
          </span>
        </Dialog.Title>

        <Dialog.Title className={'wkit-dialog__title'}>Install Wallet</Dialog.Title>
      </div>
      <div className="wkit-install">
        <img className="wkit-install__logo" src={wallet.iconUrl} alt={`${wallet.name} logo`} />
        <h1 className="wkit-install__title">You haven’t install this wallet</h1>
        <p className="wkit-install__description">Install wallet via Chrome Extension Store</p>
        <button
...
const Connecting = (props: ConnectingProps) => {
  ...
  return (
    <section>
      <div className={'wkit-dialog__header'}>
        <Dialog.Title className={'wkit-dialog__title'} style={{ margin: '-6px 12px -6px -8px' }}>
          <span className="wkit-dialog__close" onClick={props.onNavBack}>
            <SvgArrowLeft />
          </span>
        </Dialog.Title>
      ...
      <div className="wkit-connecting">
        <img className="wkit-connecting__logo" src={wallet.iconUrl} alt={`logo of ${wallet.name}`} />
        <h1 className="wkit-connecting__title">Opening {wallet.name}</h1>
        <p className="wkit-connecting__description">Confirm connection in the extension</p>
      </div>
    </section>
  );
};
    ...
    return (
      <div>
        ...
        <div className="wkit-select__scroll">
          <WalletList title={'Popular'} wallets={configuredWallets} onSelect={handleSelectWallet} />
          <WalletList title={'Others'} wallets={detectedWallets} onSelect={handleSelectWallet} />
        </div>
