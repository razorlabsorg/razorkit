...
export interface RazorKitEthChain extends Chain {
...
const mevmImolaIcon: IconMetadata = {
  iconBackground: "#ebac0e",
  iconUrl: async () => (await import("./chainIcons/mevmImola.svg")).default,
};
...
const chainMetadataByName: Record<ChainName, ChainMetadata | null> = {
  mevmImola: { chainId: 30_732, name: "Mevm Imola", ...mevmImolaIcon },
...
/** @description Decorates an array of wagmi `Chain` objects with RazorKitEthChain property overrides */
export const provideRazorKitEthChains = <Chain extends RazorKitEthChain>(chains: readonly [Chain, ...Chain[]]): Chain[] =>
  chains.map((chain) => {
